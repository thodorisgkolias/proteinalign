...
LogLikMatch = function(pair = 'NULL', data, seq, matched, vol, PAM, 
     ...
     lik = NULL
     for (i in 1:N){
          lik[i] = LogLikSizeShape(res$rot.data[, , i], res$mean, sigma, p, 
                                M1[i], A = vol)
     }
...
HungarianStep = function(data, seq, lik, lik0, matched, rows2, vol, PAM, 
     ...
     if (!is.numeric(dim(tt))) {
          ...
          liknew = LogLikMatch(pair, data, seq = seq, matched, vol,
...
RemoveStep = function(data, seq, matched, vol, lik0, PAM, gap_open, gap_ext) {
     ...
     while (remove) {
          ...
          for (i in 1:dim(hist)[1]) {
               ...
               hist2 = hist[-i, ]
               lik_remove[i] = LogLikMatch(pair = NULL, data = data,
                                           seq=seq, matched = hist2, 
                                           vol = vol, PAM = PAM, 
                                           gap_open = gap_open, 
                                           gap_ext = gap_ext)
          }
     ...
     if (!is.numeric(dim(tt))) {
          ...
          while (k <= dim(tt)[1]) {
               ...
               if (all(diff(temp[, 1]) > 0)  & all(diff(temp[, 2]) > 0)) {
                    liknew = LogLikMatch(pair, data, seq = seq, matched,
